...
- [ ] **Step 2b: Fix the now-stale agent count in the Security note.** In `docs/SUBAGENTS.md` find:
...
grep -q "The seven above are hand-authored" docs/SUBAGENTS.md && echo "OK agent count updated"
...
```bash
mkdir -p outputs/tests && touch outputs/tests/.gitkeep
```
